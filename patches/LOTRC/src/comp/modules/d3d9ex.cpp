...
#pragma region D3D9Device
	...
	HRESULT d3d9ex::D3D9Device::SetTransform(D3DTRANSFORMSTATETYPE State, CONST D3DMATRIX* pMatrix)
	{
		...
		shared::common::ffp_state::get().on_set_transform(State, pMatrix);
	...
	HRESULT d3d9ex::D3D9Device::DrawPrimitiveUP(D3DPRIMITIVETYPE PrimitiveType, UINT PrimitiveCount, CONST void* pVertexStreamZeroData, UINT VertexStreamZeroStride)
	{
		...
		return renderer::get()->on_draw_primitive_up(m_pIDirect3DDevice9, PrimitiveType, PrimitiveCount, pVertexStreamZeroData, VertexStreamZeroStride);
	}
		...
		[[maybe_unused]] UINT VertexStreamZeroStride)
	{
		...
		return renderer::get()->on_draw_indexed_prim_up(m_pIDirect3DDevice9, PrimitiveType, MinVertexIndex, NumVertices, PrimitiveCount, pIndexData, IndexDataFormat, pVertexStreamZeroData, VertexStreamZeroStride);
	}
	...
	HRESULT d3d9ex::D3D9Device::SetFVF(DWORD FVF)
	{
		...
		shared::common::ffp_state::get().on_set_fvf(FVF);
	...
	HRESULT d3d9ex::D3D9Device::SetStreamSourceFreq(UINT StreamNumber, UINT Divider)
	{
		...
		shared::common::ffp_state::get().on_set_stream_source_freq(StreamNumber, Divider);
