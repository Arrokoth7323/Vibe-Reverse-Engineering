...
namespace shared::common
{
	...
	public:
		...
		// Push current View/Proj/World to SetTransform from VS constants.
		// Called before every draw so Remix always has correct camera and object placement.
	...
	private:
		...
		// SetTransform-captured View/Proj (for games with concatenated ViewProj in VS constants)
		D3DMATRIX st_view_ = {};
		D3DMATRIX st_proj_ = {};
		...
		// ViewProj: c239-c242 (concatenated, decomposed into View + Proj)
		// World: c178-c181 (g__worldMatrix, row-major 4x4)
		// Bones: c0-c176 (g__matrixPalette, 59 bones x 3 regs, float3x4)
		// Camera: c243 (right), c244 (up), c245 (pos) — used to build View matrix
		int vs_reg_view_start_ = -1;   // -1 = not from VS constants
		int vs_reg_view_end_ = -1;
		int vs_reg_proj_start_ = -1;   // -1 = not from VS constants
		int vs_reg_proj_end_ = -1;
		int vs_reg_viewproj_start_ = 239; // concatenated ViewProj register (-1 = disabled)
		int vs_reg_cam_right_ = 243;   // g__cameraRight (-1 = disabled)
		int vs_reg_cam_up_ = 244;      // g__cameraUp (-1 = disabled)
		int vs_reg_cam_pos_ = 245;     // g__cameraPos (-1 = disabled)
